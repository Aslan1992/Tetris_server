...
public class LFigureBuilder implements FigureBuilder {
    @Override
    public List<Block> build(int y, int x, SpacePose pose) {
        switch (pose) {
            case FIRST:
                return new ArrayList<>(Arrays.asList(
                        new Block(y, x), new Block(y - 1, x), new Block(y + 1, x), new Block(y + 1, x + 1)));
            case SECOND:
                return new ArrayList<>(Arrays.asList(
                        new Block(y, x), new Block(y, x - 1), new Block(y, x + 1), new Block(y - 1, x + 1)));
            case THIRD:
                return new ArrayList<>(Arrays.asList(
                        new Block(y, x), new Block(y - 1, x), new Block(y + 1, x), new Block(y - 1, x - 1)));
            case FOURTH:
